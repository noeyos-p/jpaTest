...
public class QueryService {
    ...
    public List<Member> dynamicQuery() {
        ...
        TypedQuery/*타입이 정해져있음*/<Member> query = em.createQuery(sql, Member.class)
                .setParameter("id", "lee");
    ...
    // 멤버 테이블 중에서 씨름팀의 속해있는 멤버의 이름만 출력
    public List<Member> findMemberSsirum() {
        String sql = "SELECT m FROM Member m WHERE m.team.teamName LIKE : teamName";
        TypedQuery<Member> result = em.createQuery(sql, Member.class).setParameter("teamName", "씨름%");
